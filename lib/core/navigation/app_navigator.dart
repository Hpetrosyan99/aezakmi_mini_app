...
class AppNavigator {
  ...
  Future<void> navigatePath<T extends Object?>(String path, {OnNavigationFailure? onFailure}) {
    return _router.navigatePath(path, onFailure: onFailure);
  }

  Future<void> pushAndPopAll(PageRouteInfo route, {OnNavigationFailure? onFailure}) {
  ...
  Future<T?> showModal<T>({
    ...
    return showModalBottomSheet<T?>(
      ...
      constraints: BoxConstraints(maxHeight: context!.height - context!.topPadding - kToolbarHeight),
      backgroundColor: backgroundColor ?? context?.backgroundSurface,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(kBorderRadius16px))),
